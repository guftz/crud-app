...
app.get("/clientes/:cpf", (req, res) => {
  ...
  connection.query(query, [cpf], (erro, resultado) => {
    if (erro) return res.status(500).json({ erro: erro.message });
...
app.post("/clientes", (req, res) => {
    // Executa a Query SQL e retorna a resposta para o usuário
    const { cpf, nome, email, tel } = req.body;
    const query = "INSERT INTO clientes (cpf, nome, email, tel) VALUES(?, ?, ?, ?)";
    connection.query(query, [cpf, nome, email, tel], (erro, resultado) => {
        if (erro) return res.status(500).json({ erro: erro.message });
        res.status(201).json({ cpf, nome, email, tel });
    });
});
...
app.patch("/clientes/:cpf", (req, res) => {
  ...
  const query = `UPDATE clientes SET ${aSerMudado} WHERE cpf = ?`;

  // Executa a Query SQL e retorna a resposta para o usuário
  connection.query(query, [...values, cpf], (erro, resultado) => {
    if (erro) return res.status(500).json({ erro: erro.message });
    if (resultado.affectedRows === 0)
      return res.status(404).json({ message: "Usuário não encontrado." });
